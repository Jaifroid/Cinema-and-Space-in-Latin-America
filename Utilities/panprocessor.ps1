...
Usage: .\panprocessor FILENAME or DIRECTORY [-offset num] [-steps num] [-pad]
    ...
    [-split | -nosplit] [-linknotes] [-todocx | -tomd] [-nobom] [-help] | [-addbom] 
...
'@
    ...
    if ( $todocx) {
        ...
        $filter = 'docx' 
        $args1 = @('-o', $outfile, '-s', '-t', $filter, '--wrap=none', '--reference-doc=template.docx')
        "Writing output to $outfile ..."
        # Write-Host pandoc $filename $args1 $toc
        & pandoc $filename $args1 $toc
    } else {
...
"@  
        ...
        $args1 = @('-o', $outfile, '-s', '-t', $filter, '--wrap=none', "--extract-media=$mediadir", '--atx-headers', '--reference-location=section', '--top-level-division=chapter', '--toc', '--reference-doc=template.docx')
        "Writing output to $outfile ..."
        if ($tomaster) {
            ...
            & pandoc @($sourcedocs | % { ls -r $filename ($_ + '.md') | % { $_.FullName } }) $args1
        } else {
            ...
            & pandoc @(ls -r $outpath | Sort-Object { [regex]::Replace($_.FullName, '\d+', { $args[0].Value.PadLeft(20) }) } | % { $_.FullName }) $args1
        }
        "Done."
